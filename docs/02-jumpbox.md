...
To replicate the compute resources from the official repository, adapted to our virtual machines with Multipass, we spin up the nodes, ensuring the correct resources are allocated.

# Virtual Machine Provisioning
Run the following commands from your host machine:

multipass launch 24.04 --name server --cpus 2 --mem 2G --disk 10G
multipass launch 24.04 --name node-0 --cpus 1 --mem 1G --disk 10G
multipass launch 24.04 --name node-1 --cpus 1 --mem 1G --disk 10G
...
We install kubectl in the Windows/WSL host environment to interact with the control plane once the cluster is up and running.

In WSL2 / Linux subsystem:
